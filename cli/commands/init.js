...
export function initCommand(program) {
  program
    .command('init')
    .description('Initialize Firebase for your DDEX Distro project')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (!fs.existsSync('firebase.json')) {
          console.error(chalk.red('❌ Not in a DDEX Distro project directory'));
          process.exit(1);
        }

        console.log(chalk.cyan('🔥 Initializing Firebase for DDEX Distro\n'));
        ...
        if (apps.length === 0) {
          // Create web app
          const appName = 'DDEX Distro Web App';
        ...
        console.log(chalk.cyan('  ddex-distro deploy    # Deploy to Firebase'));
        console.log(chalk.cyan('  ddex-distro target add # Add delivery targets'));
