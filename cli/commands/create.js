...
export function createCommand(program) {
  program
    ...
    .description('Create a new DDEX Distro project')
...
`.trim();
        ...
        if (!options.skipGit) {
          ...
          execSync('git commit -m "Initial commit from DDEX Distro CLI"', { cwd: projectPath, stdio: 'ignore' });
        ...
        console.log(chalk.cyan('  ddex-distro init      # Initialize Firebase'));
        ...
        if (options.edition === 'enterprise') {
          ...
          console.log(chalk.cyan('  ddex-distro plugin install <plugin-name>'));
        }
