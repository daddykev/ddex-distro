...
export function deployCommand(program) {
  program
    .command('deploy')
    .description('Deploy your DDEX Distro platform to Firebase')
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.log(chalk.cyan('🚀 Deploying DDEX Distro to Firebase\n'));
        ...
        if (hostingUrlMatch) {
          ...
          console.log('\nYour DDEX Distro platform is live at:');
          console.log(chalk.cyan.bold(hostingUrlMatch[1]));
        }

        console.log('\n' + chalk.yellow('📝 Post-deployment checklist:'));
        console.log('  1. Configure delivery targets: ' + chalk.cyan('ddex-distro target add'));
